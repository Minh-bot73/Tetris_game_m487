...
int32_t main(void)
{
    ...
    // Configure ((GPIO_DBCTL_T *) (((uint32_t)0x40000000) + 0x04440UL)) for LEDs
    GPIO_Config();
        ...
        switch (currentState) {
            case WELCOME_SCREEN:
                ...
                if (switchPressed) {
                    ...
                    currentX = 60 // unit(10x10 for each unit); // Set spawn position
            ...
            case PAUSED:
                ...
                LCD_PutString(60, 140, (uint8_t *)"Paused", 0xFFE0, 0x0000);

                if (switchPressed) {
                    // User pressed button to resume - go back to playing state
                    currentState = PLAYING;
                    switchPressed = 0;
                    ((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x10000UL))->CTL |= (1 << 30); // Manually enable Timer0
                    ((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x10100UL))->CTL |= (1 << 30); // Manually enable Timer1
