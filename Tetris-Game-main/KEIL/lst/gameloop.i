...
void LCD_Draw_SquareBorder(uint16_t x, uint16_t y, uint16_t rows, uint16_t cols, uint16_t fillColor, uint16_t borderColor) {
    ...
    // Top row
    for (i = 0; i < cols; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y, fillColor, borderColor);
    }

    // Bottom row
    for (i = 0; i < cols; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y + (rows - 1) * 10, fillColor, borderColor);
    }

    // Left column (excluding corners)
    for (i = 1; i < rows - 1; i++) {
        LCD_Draw_FilledSquare(x, y + i * 10, fillColor, borderColor);
    }

    // Right column (excluding corners)
    for (i = 1; i < rows - 1; i++) {
        LCD_Draw_FilledSquare(x + (cols - 1) * 10, y + i * 10, fillColor, borderColor);
    }
}
...
void Draw_I_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
 int i;
    for (i = 0; i < 4; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y, 0x07FF // I shape, borderColor);
    }
}
void Draw_J_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
    LCD_Draw_FilledSquare(x, y, 0x001F, borderColor);
  int i;
    for (i = 0; i < 3; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y + 10, 0x001F, borderColor);
    }
}
void Draw_L_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
  int i;
    for (i = 0; i < 3; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y + 10, 0xFD20, borderColor);
    }
    LCD_Draw_FilledSquare(x + 2 * 10, y, 0xFD20, borderColor);
}

void Draw_S_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
  LCD_Draw_FilledSquare(x + 10, y, 0x07E0 // S shape, borderColor);
    LCD_Draw_FilledSquare(x + 2 * 10, y, 0x07E0 // S shape, borderColor);
    LCD_Draw_FilledSquare(x, y + 10, 0x07E0 // S shape, borderColor);
    LCD_Draw_FilledSquare(x + 10, y + 10, 0x07E0 // S shape, borderColor);
}
void Draw_O_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
    LCD_Draw_FilledSquare(x, y, 0xFFE0 // O shape, borderColor);
    LCD_Draw_FilledSquare(x + 10, y, 0xFFE0 // O shape, borderColor);
    LCD_Draw_FilledSquare(x, y + 10, 0xFFE0 // O shape, borderColor);
    LCD_Draw_FilledSquare(x + 10, y + 10, 0xFFE0 // O shape, borderColor);
}
void Draw_T_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
  int i;
    for (i = 0; i < 3; i++) {
        LCD_Draw_FilledSquare(x + i * 10, y, 0x780F // T shape, borderColor);
    }
    LCD_Draw_FilledSquare(x + 10, y + 10, 0x780F // T shape, borderColor);
}
void Draw_Z_Shape(uint16_t x, uint16_t y, uint16_t borderColor) {
    LCD_Draw_FilledSquare(x, y, 0xF800 // Z shape, borderColor);
    LCD_Draw_FilledSquare(x + 10, y, 0xF800 // Z shape, borderColor);
    LCD_Draw_FilledSquare(x + 10, y + 10, 0xF800 // Z shape, borderColor);
    LCD_Draw_FilledSquare(x + 2 * 10, y + 10, 0xF800 // Z shape, borderColor);
}
...
void Draw_Sidebar_UI(void) {
  ...
  uint16_t sidebarX1 = 165;
    uint16_t lineY;
    // Title: "NEXT"
    LCD_PutString(sidebarX, 25, (uint8_t *)"NEXT", 0xFFFF, 0x2104);
    // "SCORE"
    lineY = 95;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"SCORE", 0xFFFF, 0x2104);

    // "TIME"
    lineY = 145;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"TIME", 0xFFFF, 0x2104);

    // "LEVEL"
    lineY = 195;
 LCD_PutString(sidebarX, lineY, (uint8_t *)"LEVEL", 0xFFFF, 0x2104);


    // "RESTART"
    lineY = 260;
    LCD_PutString(sidebarX1, lineY, (uint8_t *)"PLAY", 0xFFFF, 0x2104);

    // "PAUSE"
    lineY = 280;
    LCD_PutString(sidebarX1, lineY, (uint8_t *)"PAUSE", 0xFFFF, 0x2104);
}
void LCD_Draw_Border(){
 ...
 LCD_Draw_SquareBorder(0, 0, 32, 24, 0x2389, 0x0000); // light gray border



 // Draw vertical grey column (20 height between play area and side panel)
 LCD_Draw_SquareColumn(15 * 10, 0, 31, 0x8922, 0x0000); // x=15 since play area is 14 + 1 spacing

 // Draw top horizontal grey row (under NEXT)
 LCD_Draw_SquareRow(15 * 10, 7 * 10, 8, 0x8922, 0x0000); // 7 squares wide

 // Draw bottom horizontal grey row (above RESTART)
 LCD_Draw_SquareRow(15 * 10, 24 * 10, 8, 0x8922, 0x0000); // 7 squares wide
 Draw_Sidebar_UI();
}
...
void LCD_DisplayTime(uint8_t min, uint8_t sec)
{
  ...
  lineY = 165;
    ...
    LCD_PutString(sidebarX, lineY, (uint8_t *)buffer, 0xFFFF, 0x2104); // Show time
}
...
void Draw_Score_Level(void) {
    ...
    LCD_PutString(sidebarX, 115, (uint8_t *)buffer, 0xFFFF, 0x2104);

    // Draw Level
    sprintf(buffer, "%u", level);
    LCD_PutString(sidebarX, 215, (uint8_t *)buffer, 0xFFFF, 0x2104);
}
