...
int isPaused = 0;
int highestScore = 0;
extern int board[32 // exclude 2 unit for top and down border][14 + 1];
extern uint32_t highScores[5];
...
void Draw_Sidebar(void) {
    ...
    uint16_t lineY;
    lineY = 11;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"NEXT", 0xF800, 0x2104);

    lineY = 81;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"HISCO", 0xF800, 0x2104);

    lineY = 125;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"SCORE", 0xF800, 0x2104);

    lineY = 175;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"TIME", 0xF800, 0x2104);

    lineY = 225;
    LCD_PutString(sidebarX, lineY, (uint8_t *)"LEVEL", 0xF800, 0x2104);

    lineY = 271;
    LCD_PutString(
        sidebarX1, lineY, (uint8_t *)"PLAY",
        isPaused ? 0xFFFF : 0xFFE0,
        0x2104
    );
...
void LCD_Draw_Border(){
 ...
 LCD_Draw_SquareBorder(0, 0, 32, 24, 0xF81F, 0x0000);


 LCD_Draw_SquareColumn(15 * 10, 0, 31, 0xF81F, 0x0000);

 LCD_Draw_SquareRow(16 * 10, 7 * 10, 7, 0xF81F, 0x0000);

 LCD_Draw_SquareRow(16 * 10, 26 * 10, 7, 0xF81F, 0x0000);
 Draw_Sidebar();
}
...
void LCD_DisplayTime(uint8_t min, uint8_t sec)
{
  ...
  lineY = 195;
    ...
    LCD_PutString(sidebarX, lineY, (uint8_t *)buffer, 0x001F, 0x2104);
}
...
void DisplayScoreLevel(void) {
    ...
    LCD_PutString(sidebarX, 145, (uint8_t *)buffer, 0x001F, 0x2104);

    sprintf(buffer, "%u", level);
    LCD_PutString(sidebarX, 245, (uint8_t *)buffer, 0x001F, 0x2104);
}
...
void DisplayHighestScore(void) {
    ...
    LCD_PutString(sidebarX, 101, (uint8_t *)buffer, 0x001F, 0x2104);
}
...
void DisplayHighScores(void) {
    ...
    LCD_PutString(50, 40, (uint8_t *)"TOP 5 SCORES", 0xFFE0, 0x0000);
    ...
    for (i = 0; i < 5; i++) {
        ...
        LCD_PutString(60, y, (uint8_t *)buffer, 0xFFFF, 0x0000);
        y += 30;
    }

    LCD_PutString(30, y + 20, (uint8_t *)"Press SW1 to try again.", 0x07E0, 0x0000);
    LCD_PutString(90, y + 50, (uint8_t *)"GOGOGO!!!", 0x07E0, 0x0000);
}


void DisplayGameOverScreen(void) {
    LCD_BlankArea(0, 0, 240 // LCD Width at pixels, 320 // LCD Hight at pixels, 0x0000);
    LCD_PutString(40, 100, (uint8_t *)"GAME OVER - NICE TRY :)", 0xFFFF, 0x0000);
    LCD_PutString(40, 140, (uint8_t *)"PRESS SW1 to skip", 0xFFE0, 0x0000);
}
