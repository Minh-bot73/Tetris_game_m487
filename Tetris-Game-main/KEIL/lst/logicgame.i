...
// Board and scoring
extern int board[32 // exclude 2 unit for top and down border][14 + 1]; // 0 = empty, 1 = filled
...
int board[32 // exclude 2 unit for top and down border][14 + 1] = {0};
...
void Rotate_Clockwise(TetrisBlock* shape) {
 ...
 int i;
    // Perform 90-degree clockwise rotation around the second block (pivot) for each TetrisBlock
    // This uses the standard rotation formula: new_x = pivot_x - (y - pivot_y), new_y = pivot_y + (x - pivot_x)
    for (i = 0; i < 4; i++) {
...
TetrisBlock CreateTetrisBlock(int shapeIndex, uint16_t pixelX, uint16_t pixelY, uint16_t color) {
    ...
    switch (shapeIndex) {
        ...
        case 5: // T-shape - T
            t.blocks[0].x = pixelX + 0*10; t.blocks[0].y = pixelY + 10;
            t.blocks[1].x = pixelX + 1*10; t.blocks[1].y = pixelY + 10;
            t.blocks[2].x = pixelX + 2*10; t.blocks[2].y = pixelY + 10;
            t.blocks[3].x = pixelX + 1*10; t.blocks[3].y = pixelY;
            break;
...
void Erase_next_Shape(int x, int y, uint16_t borderColor, TetrisBlock* nextTetrisBlock) {
    ...
    LCD_BlankArea(x, y, (4 * 10) + 5, (2 * 10) + 5, 0x10a2 // I shape);

    // Now draw the next shape in background color to ensure full erase
    *nextTetrisBlock = CreateTetrisBlock(nextShapeIndex, x, y, 0x10a2 // I shape);
    DrawTetrisBlock(*nextTetrisBlock, 0x10a2 // I shape);
}
...
void Erase_Current_Shape(int x, int y, uint16_t borderColor, TetrisBlock* currentTetrisBlock) {
    ...
    currentTetrisBlock->color = 0x10a2 // I shape; // Temporarily set to background
    DrawTetrisBlock(*currentTetrisBlock, 0x10a2 // I shape); // Draw to erase
...
void Timer1_SetIntervalByLevel(uint8_t level)
{
    ...
    ((TIMER_T *) ((((uint32_t)0x40000000) + (uint32_t)0x00040000) + 0x10100UL))->CMP = level_cmp[level]; // Set compare value for timer
}
// Function to update the player's score and level based on points earned
void UpdateScore(uint32_t points) {
    score += points; // Add points to total score
    uint8_t newLevel = (score / 5) + 1; // Calculate new level
    if (newLevel > 10) newLevel = 10; // Cap at max level
    if (newLevel != level) {
...
int Check_Collision(TetrisBlock *t) {
    int i;
    for (i = 0; i < 4; i++) {
        // Convert pixel coordinates to grid coordinates (each square is 10 pixels)
        int gridX = t->blocks[i].x / 10; // Convert pixel x to grid x
        int gridY = t->blocks[i].y / 10; // Convert pixel y to grid y

           if (gridX <= 0 || gridX >= 14 + 1 || gridY <= 0 || gridY >= 32 // exclude 2 unit for top and down border - 1) // Check boundary collision
            return 1;
...
void Lock_TetrisBlock_To_Board(TetrisBlock* t) {
 int i;
    for (i = 0; i < 4; i++) {
        int gridX = t->blocks[i].x / 10; // Convert to grid coordinates
        int gridY = t->blocks[i].y / 10;
                        if (gridY >= 1 && gridY <= 32 // exclude 2 unit for top and down border && gridX >= 1 && gridX <= 14) { // Ensure within board bounds
...
int CheckAndClearLines() {
    ...
    for (row = 32 // exclude 2 unit for top and down border - 2; row >= 1; row--) {
        ...
        // Check columns 1 to 14 for emptiness
...
void HardDrop() {
    ...
    while (1) {
        ...
        for (i = 0; i < 4; i++) {
            temp.blocks[i].y += 10; // Simulate drop
        }
        // Check collision with the rotated shape
        if (Check_Collision(&temp)) break; // Stop if collision
        // Erase using rotated state
        Erase_Current_Shape(currentX, currentY, 0x10a2 // I shape, &currentTetrisBlock);
        currentY += 10; // Update position
...
void MoveTetrisBlockDown() {
    ...
    // Move all blocks of the temporary tetris block down by 10
    for (i = 0; i < 4; i++) {
        temp.blocks[i].y += 10; // Simulate downward movement
    }
...
void MoveTetrisBlockLeft() {
    ...
    if (!Check_Collision(&temp)) { // If no collision
        Erase_Current_Shape(currentX, currentY, 0x10a2 // I shape, &currentTetrisBlock); // Erase old
        currentX -= 10; // Update position
...
void GetShadowPosition(TetrisBlock *currentTetrisBlock, int *shadowY) {
    ...
    while (1) {
        ...
        for (i = 0; i < 4; i++) {
            temp.blocks[i].y += 10; // Move down
        }
        if (Check_Collision(&temp)) break; // Stop on collision
        y += 10; // Update shadow Y
...
void Draw_Shadow_TetrisBlock(int x, int y, TetrisBlock *tetrisBlock) {
    ...
    for (i = 0; i < 4; i++) {
        LCD_Draw_FilledSquare(shadow.blocks[i].x, shadow.blocks[i].y, 0xC618, 0x0000); // Draw shadow squares
    }
}
...
void DelayMs(uint32_t ms) {
    ...
    while (ms--) { // Loop for each millisecond
        for (count = 0; count < 8000; count++) { // Inner loop calibrated for ~1ms
            __builtin_arm_nop(); // No Operation (adjust 8000 for your MCU speed)
        }
    }
}
